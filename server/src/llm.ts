...
export interface ChatRequest {
    ...
    noteContent?: string;
    mode?: 'chat' | 'summarize' | 'rewrite' | 'structure';
}
...
import { webRagService } from './services/webRagService';

export async function handleChat(
    messages: { role: 'user' | 'assistant' | 'system'; content: string }[],
    noteContent: string | undefined,
    mode: 'chat' | 'summarize' | 'rewrite' | 'structure' | undefined,
    config: LLMProvider,
    ...
    if (useSearch && config.searxngUrl) {
        ...
        if (lastUserMessage) {
            ...
            try {
                const searchResults = await searchWeb(lastUserMessage.content, config.searxngUrl);
                console.log(`[LLM] Raw Search results: ${searchResults.length}`);

                let webContext = "";
                // Use WebRAG if possible
                if (searchResults.length > 0) {
                    webContext = await webRagService.enrichWebResults(lastUserMessage.content, searchResults, config);
                } else {
    ...
    if (config.type === 'openai' || config.type === 'custom') {
        if (!config.apiKey && config.type === 'openai') throw new Error("OpenAI API Key is missing. Please configure it in the settings.");
